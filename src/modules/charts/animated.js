...
var AnimatedBarChart = /*#__PURE__*/function () {
  ...
  (0, _createClass2["default"])(AnimatedBarChart, [{
    ...
    value: function render(data) {
      ...
      var el = document.querySelector("#graphicContainer");
      this.config = {
      ...
      var setup = d3.select("#graphicContainer").select("svg").empty();

      this._create(data, this.config, setup);
    }
  }, {
    key: "_create",
    value: function _create(data, config) {
      ...
      var domain = Array.from(new Set(database.map(function (item) {
        return item.category;
      })));
      var min = d3.min(database, function (d) {
        return d.step;
      });
      console.log(min);
      var start = min;
      var max = d3.max(database, function (d) {
        return d.step;
      });
      var total = max - min;
      ...
      x.domain([0, d3.max(database, function (d) {
        return d.value;
      })]);
      color.domain(domain);

      function drawChart(cat) {
        var filteredData = database.filter(function (d) {
          return d.step === cat;
        });
        ...
        var bars = svg.selectAll(".bar").data(filteredData, function (d) {
          return d.category;
        });
        var labels = svg.selectAll(".label").data(filteredData, function (d) {
          return d.category;
        });
        var t = d3.transition("blah").duration(750); //Exit

        bars.exit().transition(t).style("opacity", "0").attr("y", height + margin.top + margin.bottom).remove();
        labels.exit().transition(t).style("opacity", "0").attr("y", height + margin.top + margin.bottom).remove(); //Update

        bars.transition(t).attr("width", function (d) {
          return x(d.value);
        }).attr("y", function (d) {
          return y(d.category);
        });
        labels.transition(t).attr("y", function (d) {
          return y(d.category) + y.bandwidth() / 2 + 5;
        }).attr("x", 20).text(function (d) {
          return d.category + " - " + (d.value / 1000).toFixed(1) + "k";
        }); //Enter

        bars.enter().append("rect").attr("class", "bar").attr("title", function (d) {
          return d.category;
        }).attr("x", 0).attr("y", height + margin.top + margin.bottom).attr("opacity", 1).attr("height", y.bandwidth()).attr("fill", function (d) {
          return color(d.category);
        }).attr("width", function (d) {
          return x(d.value);
        }).transition(t).attr("width", function (d) {
          return x(d.value);
        }).attr("y", function (d) {
          return y(d.category);
        });
        labels.enter().append("text").attr("class", "label").attr("x", 20).attr("y", height + margin.top + margin.bottom + 20).attr("opacity", 1).transition(t).attr("y", function (d) {
          return y(d.category) + y.bandwidth() / 2 + 5;
        }).text(function (d) {
          return d.category + " - " + (d.value / 1000).toFixed(1) + "k";
        });
      ...
      playButton.on("click", function () {
        var buttonStatus = playButton.text();
