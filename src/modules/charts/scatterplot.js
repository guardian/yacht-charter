...
var ScatterPlot = /*#__PURE__*/function () {
  ...
  (0, _createClass2["default"])(ScatterPlot, [{
    ...
    value: function _render(d3) {
      ...
      width = width - self.margin.left - self.margin.right, height = height - self.margin.top - self.margin.bottom; // Filter the data if the filter value has been set in the Googledoc
      ...
      var svg = d3.select("#graphicContainer").append("svg").attr("width", width + self.margin.left + self.margin.right).attr("height", height + self.margin.top + self.margin.bottom).attr("viewBox", "0 0 ".concat(width + self.margin.left + self.margin.right, " ").concat(height + self.margin.top + self.margin.bottom)).classed("svg-content", true).append("g").attr("transform", "translate(" + self.margin.left + "," + self.margin.top + ")"); // Get the values for the X Axis using all the values from the database (This means you can flip between categories and compare values on the same axis)
